...
A Phase 3 reference arrives as a .gds design file instead of an imaged
crop; pass the path and it is rendered on the way in (see cad_render.py).
"""
...
GDS_EXTS = {".gds", ".gds2", ".gdsii"}
REF_EXTS = IMAGE_EXTS | GDS_EXTS
CSV_FIELDS = ("sample_id", "pred_x", "pred_y", "confidence", "inference_time_ms")
ID_KEYS = ("sample_id", "id", "sample", "name")
REF_KEYS = ("reference_gds_path", "reference_path", "reference", "ref",
...
def load_reference(reference, spot_nm: float = cad_render.BEAM_SPOT_NM,
                   cd_bias_px: float = 0.0) -> np.ndarray:
    """Reference -> grayscale array, whichever form it arrived in.

    A .gds is a design database with no brightness in it, so it is
    rasterized through the yield model; an image is read as-is.
    """
    if isinstance(reference, np.ndarray):
        return reference
    if cad_render.is_gds_path(reference):
        return cad_render.render_gds(reference, spot_nm=spot_nm,
                                     cd_bias_px=cd_bias_px)
    img = cv2.imread(str(reference), cv2.IMREAD_GRAYSCALE)
    if img is None:
...
def predict(reference_image_path, search_image_path, model=None, device=None,
            sample_id: str | None = None, weights_dir=None,
            spot_nm: float = cad_render.BEAM_SPOT_NM,
            cd_bias_px: float = 0.0, apply_gate: bool = True,
            prior_xy: tuple[float, float] | None = None,
            match: str = train.MATCH_GREY,
            scales: tuple[float, ...] | None = None,
            angles: tuple[float, ...] | None = None,
            lock_scale: bool = False,
            load_weights: bool = True) -> dict:
    """Run the matcher on one pair. No training.

    Returns sample_id, pred_x, pred_y, found, confidence, inference_time_ms.
    Origin is the top-left of the search image. The reference may be a PNG,
    a .gds design file, or an already-loaded array.
    """
    if load_weights and (model is None or device is None):
        model, device = load_model(weights_dir)
    elif device is None:
        device = train.pick_device()
    sea_path = Path(search_image_path)
    ref = load_reference(reference_image_path, spot_nm=spot_nm,
                         cd_bias_px=cd_bias_px)
    sea = cv2.imread(str(sea_path), cv2.IMREAD_GRAYSCALE)
    if sea is None:
        raise FileNotFoundError(f"could not read search {sea_path}")
    t0 = time.perf_counter()
    out = train.localize_pair(ref, sea, model, device, apply_gate=apply_gate,
                              prior_xy=prior_xy, match=match,
                              scales=scales, angles=angles,
                              lock_scale=lock_scale)
    elapsed_ms = (time.perf_counter() - t0) * 1000.0
    if sample_id is not None:
        sid = sample_id
    elif isinstance(reference_image_path, np.ndarray):
        sid = sea_path.stem
    else:
        sid = Path(reference_image_path).stem
    return {
        "sample_id": str(sid),
        "pred_x": float(out["x"]),
        "pred_y": float(out["y"]),
...
def list_pairs(ref_dir: Path, sea_dir: Path) -> list[tuple[str, Path, Path]]:
    refs = sorted(
        ...
        and not p.name.endswith("_preview.png")
    )
    pairs, missing = [], []
    for ref_path in refs:
        if ref_path.suffix.lower() in GDS_EXTS:
            sea_path = next((sea_dir / f"{ref_path.stem}{ext}"
                             for ext in IMAGE_EXTS
                             if (sea_dir / f"{ref_path.stem}{ext}").is_file()),
                            None)
        else:
